...
class ESRFMeas(XRDMeas):
    ...
    def __init__(
        self, raw_h5_path: pl.Path, processed_h5_path: pl.Path, group_name: str
    ):
    ...
    def _read_position(self, raw_grp):
        ...
        x_pos = 0.0 if x_pos == 0.0 else x_pos
        y_pos = 0.0 if y_pos == 0.0 else y_pos

        self.x_position = x_pos * mu.mm
        self.y_position = y_pos * mu.mm

    def _read_raw_metadata(self, raw_grp):
        inst = raw_grp.get("instrument")
        if inst is None:
            return
    ...
    def _read_raw_measurement(self, raw_grp):
    ...
    def _read_cdte_image(self, meas_grp, subname):
        try:
            item = meas_grp[subname]
            if isinstance(item, h5py.Dataset):
    ...
    def _read_processed_data(self, proc_grp):
    ...
    def _read_h5_value(dataset):
        ...
        normalised = ESRFMeas.UNIT_ALIASES.get(unit_str, unit_str)
    ...
    # Results part
    def add_results(self, results_path: pl.Path):
        """
        Parameters
        ----------
        results_path : Path
    ...
    def _results_base_path(self) -> pl.Path:
        ...
        fullpath = self._results_path / f"{self._results_path.stem}_{self._group_name}"
        return fullpath
