...
class XRDMeas(BaseMeasurement):
    ...
    Subclasses can override _results_base_path to redirect where the
    result files are searched (e.g. ESRF stores them in PROCESSED_DATA/).
    """

    @property
    def _results_base_path(self) -> pl.Path:
        """Base path used to locate .lst, .par and .dia result files.
        Defaults to self.path — override in subclasses if results live elsewhere."""
        return self.path
    ...
    def _parse_dia(self):
        ...
        two_theta, measured, calculated, background = [], [], [], []
        phase_patterns = {}
